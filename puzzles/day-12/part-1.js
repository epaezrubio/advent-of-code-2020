...
function transposeOrientation (current, direction, rotation) {
    let directionIncement = (direction === 'R' ? 1 : -1) * rotation / 90;
    let nextDirection = orientations.indexOf(current) + directionIncement
...
function getShipTravelDistance (instructions) {
    ...
    instructions.forEach(([instruction, value]) => {
        ...
        if (instruction === 'F') {
            // forward translation
            let increment = translationComponents[orientation]
